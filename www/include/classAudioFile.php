...
class AudioFile
{
	...
	//id tag?-------------------------------
		...
		if(substr($TAG,0,3) == "TAG")
		{
			...
			$info["mpeg_id3v1_tag"]["lyrics"] 	= rtrim(substr($TAG,119,6));

			$info["mpeg_id3v1_tag"]["genre"]	= "";

			$info["mpeg_id3v1_tag"]["copyright"]=rtrim(substr($TAG,28,30));
...
$this->id3_lyrics = $info["mpeg_id3v1_tag"]["lyrics"];
		}
	}
	...
	{	
		...
		if ($this->id3_tag)
		{
			...
			$info.="-".$this->id3_year;
                        ...
                        $info.="-".date("i:s", mktime(0,0,round($this->wave_length)));

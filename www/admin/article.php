...
if($action=="savecat"){
mysql_query("select * from categories where categories_id='".$cat."'");
if(mysql_affected_rows()>0){
mysql_query("update categories set categories_title='".$categories_title."', categories_description='".$categories_description."' where categories_id='".$cat."'");
}else{
mysql_query("insert into categories set categories_title='".$categories_title."', categories_description='".$categories_description."',categories_id='".$cat."'");
}
}
...
<br><small>Article Link :</small><br><input type=text name='article_link' size=60 value='".stripslashes($r['articles_link'])."'><br><br>
...
if($r=mysql_fetch_array($q)){
echo "<form action='article.php?action=savecat' method='POST' ENCTYPE='multipart/form-data'>
<small>Category Title :</small><br><input type=text name='categories_title' size=60 value=\"".stripslashes($r['categories_title'])."\">
<br><br><small>Category content :</small><br><textarea cols=60 rows=10 name='categories_description'>".stripslashes($r['categories_description'])."</textarea><br>
<input type=hidden name=cat value='$cat'><br><input type=submit value='::: Update :::'></form>";
}
}
}
